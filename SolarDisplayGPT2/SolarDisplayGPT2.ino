...
// ------------------------- Layout (Basis 320x240, skaliert dynamisch) -------------------------
static int   W=320, H=240;
static float layoutScale = 1.0f;
static int   PAD_X=8, PAD_Y=6;
static int   STATUS_H=32;
...
static inline int scaleText(int base){
  return max(1, (int)lroundf(base * layoutScale));
}

static void updateLayoutFromDisplay(LGFX& disp){
  W = disp.width();
  H = disp.height();
  layoutScale = min(W/320.0f, H/240.0f);
  PAD_X = scaleDim(8);
  PAD_Y = scaleDim(6);
  STATUS_H = scaleDim(32);
  headerLineY = STATUS_H + scaleDim(2);
  startY = headerLineY + scaleDim(6);
  SWIPE_MIN_DIST = scaleDim(60);
  TAP_MAX_DIST = scaleDim(15);
}
...
static inline void drawStatusHeader(LGFX& tft, const PvFrameV4& f){
  ...
  // Zeit links
  tft.setTextDatum(ML_DATUM); tft.setTextFont(2); tft.setTextSize(scaleText(2)); tft.setTextColor(fg,bg);
  tft.drawString(nowHHMM(), PAD_X, STATUS_H/2);

  // Batterie Mitte
  const int iconW=scaleDim(60), iconH=scaleDim(24);
  const int iconX=W/2-iconW/2-scaleDim(25), iconY=STATUS_H/2-iconH/2;
  ...
  tft.setTextDatum(MC_DATUM); tft.setTextFont(1); tft.setTextSize(scaleText(2)); tft.setTextColor(TFT_GREEN);
  tft.drawString(String(soc)+"%", iconX+iconW/2, iconY+iconH/2);

  // ETA rechts
  tft.setTextDatum(MR_DATUM); tft.setTextFont(1); tft.setTextSize(scaleText(2)); tft.setTextColor(fg,bg);
...
static inline void drawPage2Content(LGFX& tft, const PvFrameV4& f){
  ...
  const int barHeight  = scaleDim(28);
  const int gapY       = scaleDim(34);

  const int yTitle = startY + scaleDim(10);
  const int y1     = startY + scaleDim(34);
  ...
  tft.setTextFont(2); tft.setTextSize(scaleText(1)); tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("PV-String Leistung", PAD_X, yTitle);

  tft.setTextDatum(MR_DATUM);
  tft.setTextFont(2); tft.setTextSize(scaleText(1)); tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  ...
  // Unterzeile V/A
  tft.setTextFont(2); tft.setTextSize(scaleText(1)); tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.drawString(String(pv1V,1)+" V  ·  "+String(pv1A,2)+" A", barMarginX + barWidth, y1 + barHeight + scaleDim(12));
  ...
  // Unterzeile V/A
  tft.setTextFont(2); tft.setTextSize(scaleText(1)); tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.drawString(String(pv2V,1)+" V  ·  "+String(pv2A,2)+" A", barMarginX + barWidth, y2 + barHeight + scaleDim(12));
...
static inline void drawPage3Content(LGFX& tft, const PvFrameV4&){
  ...
  // Zeit groß
  tft.setTextFont(4);
  tft.setTextSize(scaleText(2));
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(nowHHMM(), W/2, headerLineY + (H - headerLineY)/2 - scaleDim(16));

  // Datum darunter
  tft.setTextFont(2);
  tft.setTextSize(scaleText(1));
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  tft.drawString(fmtDateDDMMYYYY(), W/2, headerLineY + (H - headerLineY)/2 + scaleDim(16));
}

// Seite 5 – Drei Zeiger-Gauges: PV (Reg 32064), Batterie (±), Grid (±)
static void drawPage5Content(LGFX& tft, const PvFrameV4& f){
  // ---------- Geometrie ----------
  const int gaugesY = scaleDim(42);               // Oberkante der 3 Meter
  const int gW = scaleDim(100), gH = scaleDim(160);         // Größe pro Meter
  const int xPV   =   scaleDim(5);
  const int xBATT = scaleDim(110);
  const int xGRID = scaleDim(215);
  ...
  auto drawLabel = [&](int xLeft, int y, const char* label, const String& val, uint16_t col=TFT_LIGHTGREY){
    ...
    tft.setTextFont(2); tft.setTextSize(scaleText(1));
    tft.setTextColor(col, TFT_BLACK);
    tft.drawString(label, xLeft, y);
    int lw = tft.textWidth(label);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.drawString(val, xLeft + lw + scaleDim(6), y);
  };
  auto drawCentered = [&](int xCenter, int y, const String& val, uint16_t col){
    tft.setTextDatum(MC_DATUM);
    tft.setTextFont(2); tft.setTextSize(scaleText(1));
    tft.setTextColor(col, TFT_BLACK);
    tft.drawString(val, xCenter, y + scaleDim(10));
  };
  ...
  auto drawGauge = [&](int x, int y, int w, int h,
    ...
    spr.setSwapBytes(true);   // Farbreihenfolge an das Display anpassen (relevant bei 480x320)
  ...
  drawCentered(xPV + gW/2, gaugesY + gH + scaleDim(2) - scaleDim(30), fmtKWh(pvToday), TFT_YELLOW);

  // Batt: WR-Temperatur, Load (W)
  drawLabel(xBATT, gaugesY + gH +  scaleDim(2) - scaleDim(30), "Temp",   fmtTemp(tempC),   TFT_LIGHTGREY);
  drawLabel(xBATT, gaugesY + gH + scaleDim(16) - scaleDim(30), "Load",   fmtW(loadW),      TFT_LIGHTGREY);
  ...
  int gy = gaugesY + gH + scaleDim(2) - scaleDim(30);
  tft.setTextDatum(TL_DATUM);
  tft.setTextFont(2); tft.setTextSize(scaleText(1));
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString((t1>=0.f)? fmtKWh(t1) : String("--"), xGRID + scaleDim(28), gy); gy += scaleDim(14);

  tft.setTextColor(TFT_BLUE,  TFT_BLACK);
  tft.drawString("T2", xGRID, gy);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString((t2>=0.f)? fmtKWh(t2) : String("--"), xGRID + scaleDim(28), gy); gy += scaleDim(14);

  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.drawString("Exp", xGRID, gy);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(fmtKWh(expK), xGRID + scaleDim(28), gy);
...
static void drawPage6Content(LGFX& tft, const PvFrameV4& , int kind) {
  ...
  // ---- Geometrie / Layout ----
  const int PAD_HIST = scaleDim(6);
  const int AXIS_W = scaleDim(46);          // linke Achsenspalte (mehr Platz für größere Y-Beschriftung)
  const int topY  = headerLineY + scaleDim(10);           // unterhalb Statusheader
  const int botPad = scaleDim(30);          // Platz für Legende/Marks
  const int left = PAD_HIST + AXIS_W, right = W - PAD_HIST;
  ...
  if (!have || n<=0) {
    ...
    int legY = bottom + scaleDim(6);
    tft.setTextDatum(TL_DATUM);
    tft.setTextFont(2); tft.setTextSize(scaleText(1));
    tft.setTextColor(TFT_RED,   TFT_BLACK); tft.drawString("T1",  left,      legY);
    tft.setTextColor(TFT_BLUE,  TFT_BLACK); tft.drawString("T2",  left+scaleDim(34),   legY);
    tft.setTextColor(TFT_GREEN, TFT_BLACK); tft.drawString("Exp", left+scaleDim(68),   legY);
    return;
  }
  ...
  const int gap = max(1, scaleDim(1));                                // kleiner Abstand
...
void setup(){
  ...
  tft.setSwapBytes(true);   // Farbreihenfolge für alle Farbwerte vereinheitlichen
  tft.fillScreen(TFT_BLACK);
  updateLayoutFromDisplay(tft);
  SCREEN_W = W;
  SCREEN_H = H;
