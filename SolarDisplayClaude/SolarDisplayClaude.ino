// ===========================================================================================
// SolarDisplayClaude - LovyanGFX Version für Huawei Sun2000 Monitoring
// ===========================================================================================
// Basiert auf SolarDisplay2, umgestellt auf LovyanGFX für bessere Hardware-Unterstützung
//
...
// - Unterstützt verschiedene CYD-Displays via CYD_Display_Config.h
// ===========================================================================================
...
#include "CYD_Display_Config.h"  // Externe Config für verschiedene CYD-Displays
...
static inline void drawStatusHeader(LGFX& lcd, const PvFrameV4& f){
  ...
  lcd.fillRect(0,0,W,STATUS_H,bg);

  // Zeit links
  lcd.setTextDatum(ML_DATUM); lcd.setTextFont(2); lcd.setTextSize(2); lcd.setTextColor(fg,bg);
  lcd.drawString(nowHHMM(), PAD_X, STATUS_H/2);

  // Batterie Mitte
  const int iconW=60, iconH=24;
  const int iconX=W/2-iconW/2-25, iconY=STATUS_H/2-iconH/2;
  lcd.drawRect(iconX,iconY,iconW,iconH,fg);
  lcd.fillRect(iconX+iconW,iconY+iconH/4,4,iconH/2,fg);
  int soc = f.socx10/10; int fillW=((iconW-4)*constrain(soc,0,100)/100);
  lcd.fillRect(iconX+2,iconY+2,fillW,iconH-4,fg);
  lcd.setTextDatum(MC_DATUM); lcd.setTextFont(1); lcd.setTextSize(2); lcd.setTextColor(TFT_GREEN);
  lcd.drawString(String(soc)+"%", iconX+iconW/2, iconY+iconH/2);

  // ETA rechts
  lcd.setTextDatum(MR_DATUM); lcd.setTextFont(1); lcd.setTextSize(2); lcd.setTextColor(fg,bg);
  lcd.drawString(String("ETA: ")+fmtETA(f.eta20s), W-PAD_X, STATUS_H/2);

  // Linie
  lcd.drawLine(PAD_X, headerLineY, W-PAD_X, headerLineY, TFT_DARKGREY);
}
...
static inline void drawPage2Content(LGFX& lcd, const PvFrameV4& f){
  ...
  auto drawHBar = [&](int x, int y, int w, int h, int32_t valueW, int32_t maxW, uint16_t colFill){
    ...
    if (fillW > 0) lcd.fillRect(x+1, y+1, fillW, h-2, colFill);
  };
  ...
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString("PV-String Leistung", PAD_X, yTitle);

  lcd.setTextDatum(MR_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  lcd.drawString("PV1/PV2 max 6.0 kW", W-PAD_X, yTitle);
  ...
  lcd.setTextDatum(ML_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  lcd.drawString("PV1", barMarginX, y1 - 4);
  // Wert rechts
  lcd.setTextDatum(MR_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString(fmtWatt(pv1W), barMarginX + barWidth, y1 + barHeight/2);
  // Unterzeile V/A
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_CYAN, TFT_BLACK);
  lcd.drawString(String(pv1V,1)+" V  ·  "+String(pv1A,2)+" A", barMarginX + barWidth, y1 + barHeight + 12);
  ...
  lcd.setTextDatum(ML_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  lcd.drawString("PV2", barMarginX, y2 - 4);
  // Wert rechts
  lcd.setTextDatum(MR_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString(fmtWatt(pv2W), barMarginX + barWidth, y2 + barHeight/2);
  // Unterzeile V/A
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_CYAN, TFT_BLACK);
  lcd.drawString(String(pv2V,1)+" V  ·  "+String(pv2A,2)+" A", barMarginX + barWidth, y2 + barHeight + 12);
  ...
  lcd.setTextDatum(ML_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  lcd.drawString("Total", barMarginX, y3 - 4);
  // Wert rechts
  lcd.setTextDatum(MR_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString(fmtWatt(pvTotalW), barMarginX + barWidth, y3 + barHeight/2);
  // kleine Skalen-Notiz für Total rechts oben vom Balken
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  lcd.drawString("max 9.0 kW", barMarginX + barWidth, y3 - 4);
}
...
static inline void drawPage3Content(LGFX& lcd, const PvFrameV4&){
  ...
  lcd.drawString(nowHHMM(), W/2, headerLineY + (H - headerLineY)/2 - 16);

  // Datum darunter
  lcd.setTextFont(2);
  lcd.setTextSize(1);
  lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  lcd.drawString(fmtDateDDMMYYYY(), W/2, headerLineY + (H - headerLineY)/2 + 16);
}
...
static void drawPage5Content(LGFX& lcd, const PvFrameV4& f){
  ...
  auto drawLabel = [&](int xLeft, int y, const char* label, const String& val, uint16_t col=TFT_LIGHTGREY){
    ...
    lcd.drawString(label, xLeft, y);
    int lw = lcd.textWidth(label);
    lcd.setTextColor(TFT_WHITE, TFT_BLACK);
    lcd.drawString(val, xLeft + lw + 6, y);
  };
  auto drawCentered = [&](int xCenter, int y, const String& val, uint16_t col){
    lcd.setTextDatum(MC_DATUM);
    lcd.setTextFont(2); lcd.setTextSize(1);
    lcd.setTextColor(col, TFT_BLACK);
    lcd.drawString(val, xCenter, y+10);
  };
  ...
  lcd.drawString("T1", xGRID, gy);
  lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString((t1>=0.f)? fmtKWh(t1) : String("--"), xGRID + 28, gy); gy += 14;

  lcd.setTextColor(TFT_BLUE,  TFT_BLACK);
  lcd.drawString("T2", xGRID, gy);
  lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString((t2>=0.f)? fmtKWh(t2) : String("--"), xGRID + 28, gy); gy += 14;

  lcd.setTextColor(TFT_GREEN, TFT_BLACK);
  lcd.drawString("Exp", xGRID, gy);
  lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  lcd.drawString(fmtKWh(expK), xGRID + 28, gy);
...
static void drawPage6Content(LGFX& lcd, const PvFrameV4& , int kind) {
  ...
  // Hintergrund & Rahmen
  lcd.fillRect(0, top, W, bottom - top, TFT_BLACK);
  lcd.drawRect(left-1, top-1, plotW+2, plotH+2, TFT_DARKGREY);
  ...
  if (!have || n<=0) {
    ...
    lcd.drawString("Keine 30-Tage-Historie vorhanden", W/2, (top+bottom)/2);

    int legY = bottom + 6;
    lcd.setTextDatum(TL_DATUM);
    lcd.setTextFont(2); lcd.setTextSize(1);
    lcd.setTextColor(TFT_RED,   TFT_BLACK); lcd.drawString("T1",  left,      legY);
    lcd.setTextColor(TFT_BLUE,  TFT_BLACK); lcd.drawString("T2",  left+34,   legY);
    lcd.setTextColor(TFT_GREEN, TFT_BLACK); lcd.drawString("Exp", left+68,   legY);
    return;
  }
  ...
  lcd.drawLine(left, zeroY, right, zeroY, TFT_LIGHTGREY);
  ...
  for (int i=0;i<n;++i) {
    ...
    if (expv[i] > 0.0f) {
      ...
      int hPix = zeroY - yTop;
      if (hPix > 0) lcd.fillRect(x, yTop, barW, hPix, COL_EXP);
    }
    ...
    if (t2k > 0.0f) {
      ...
      if (hT2 > 0) lcd.fillRect(x, zeroY+1, barW, hT2-1, COL_T2);
    }
    if (t1k > 0.0f) {
      float sum = t2k + t1k;
      ...
      if (hT1 > 0) lcd.fillRect(x, yBotT2, barW, hT1, COL_T1);
    }
    ...
    if (i == idxToday) {
      if (expv[i] > 0.0f) {
        int yTop = yFromPos(expv[i]);
        int hPix = zeroY - yTop;
        if (hPix > 0) lcd.drawRect(x, yTop, barW, hPix, COL_OUTLINE);
      }
      float sumImp = t1k + t2k;
      if (sumImp > 0.0f) {
        int yBot = yFromNeg(sumImp);
        int hPix = yBot - zeroY;
        if (hPix > 0) lcd.drawRect(x, zeroY+1, barW, hPix-1, COL_OUTLINE);
      }
    }
  ...
  // ---- Y-Labels (größer) ----
  lcd.setTextDatum(MR_DATUM);
  lcd.setTextFont(2); lcd.setTextSize(2);                 // größer als zuvor
  lcd.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  { char b[20]; dtostrf(maxExp,0,(maxExp<10.f?1:0),b); lcd.drawString(String(b), left-AXIS_W/2 +15, top,    1); }
  lcd.drawString("kWh", left-AXIS_W/2+15, zeroY, 1);
  { char b[20]; dtostrf(maxImp,0,(maxImp<10.f?1:0),b); lcd.drawString(String(b), left-AXIS_W/2 +15, bottom, 1); }

  // ---- X-Marks alle 5 Balken ----
  for (int i=0, xx=left; i<n; ++i, xx += (barW+gap)) {
    if (i % 5 == 0) lcd.drawLine(xx, bottom+1, xx, bottom+5, TFT_DARKGREY);
  }
  ...
  lcd.setTextColor(TFT_BLUE,  TFT_BLACK); lcd.drawString("T2",  left+34,   legY);
  lcd.setTextColor(TFT_GREEN, TFT_BLACK); lcd.drawString("Exp", left+68,   legY);
  lcd.setTextColor(TFT_YELLOW,TFT_BLACK); lcd.drawString(String(kosten), left+130,   legY);
}
...
LGFX lcd;  // LovyanGFX Display-Instanz (definiert in CYD_Display_Config.h)
...
// Touch/Swipe (LovyanGFX integriert)
static const int SCREEN_W=320, SCREEN_H=240;
static const int SWIPE_MIN_DIST=60, SWIPE_MAX_TIME=1000, TAP_MAX_DIST=15;
...
// ================= Touch-Verarbeitung (LovyanGFX) =================
static bool readTouchAvg(int &x, int &y) {
  // LovyanGFX Touch-API: lcd.getTouch() gibt bereits kalibrierte Koordinaten
...
void setup(){
  ...
  lcd.setTextFont(2); lcd.setTextSize(1); lcd.setTextColor(TFT_WHITE, TFT_BLACK);
#ifdef ROLE_POLLER
  lcd.drawString("Poller bereit…", 160, 120);
#else
  lcd.drawString("Warte auf PV-Daten…", 160, 120);
#endif
}
